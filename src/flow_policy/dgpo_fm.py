...
class DGPOFMState:
    ...
    def _step_minibatch(self, transitions: DGPOFMTransition, prng: Array) -> tuple[DGPOFMState, dict[str, Array]]:
        ...
        # 1. 估算 Targets 并生成动作池 (注意：这里不再返回 metrics 和 pool_weights)
        pool_actions, target_qs = self._compute_targets(transitions, obs_norm, prng_targets)
        ...
        # ==========================================
        # 👑 3. 用刚刚进化完毕的 new_value_params 重新打分！
        # ==========================================
