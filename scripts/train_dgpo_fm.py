...
from flow_policy import dgpo_fm, rollouts
...
def main(
    ...
    config: dgpo_fm.DGPOFMConfig,
    exp_name: str = "",
    seed: int = 0,
) -> None:
    """Main function to train DGPO (Flow Matching) on a specified environment."""
    ...
    agent_state = dgpo_fm.DGPOFMState.init(prng=jax.random.key(seed), env=env, config=config)
