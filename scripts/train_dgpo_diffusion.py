...
from flow_policy import dgpo_diffusion, rollouts
...
def main(
        ...
        config: dgpo_diffusion.DGPODiffusionConfig,
        exp_name: str = "",
        seed: int = 0,
) -> None:
    """Main function to train DGPO (Diffusion) on a specified environment."""
    ...
    agent_state = dgpo_diffusion.DGPODiffusionState.init(prng=jax.random.key(seed), env=env, config=config)
